...
class Sage_AI_Core_Gen_Admin_Settings {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'sage_ai_embeddings_post_type' ) );
	...
	public function sage_ai_embeddings_post_type() {

		$labels = array(
			'name'            => 'Sage AI Embeddings',
			'singular_name'   => 'Sage AI Embedding',
			'menu_name'       => 'Sage AI Embeddings',
			'public'          => false,
			'show_ui'         => false,
			'show_in_menu'    => false,
		...
		register_post_type( 'sage-ai-embeddings', $labels );
	}
	...
	public function register_sub_menu() {
		...
		// adding bulk content gen page
		add_submenu_page( 'wp-ai-content-gen-settings', 'Chatbot', 'Chatbot', 'manage_options', 'wp_ai_content_chatbot', array( &$this, 'wp_sage_ai_chatbot' ) );

		// adding AI Images
		add_submenu_page( 'wp-ai-content-gen-settings', 'AI Images', 'AI Images', 'manage_options', 'wp_sage_ai_images', array( &$this, 'wp_sage_ai_images' ) );

		// adding Embeddings
		add_submenu_page( 'wp-ai-content-gen-settings', 'Embeddings', 'Embeddings', 'manage_options', 'wp_sage_ai_embeddings', array( &$this, 'wp_sage_ai_embeddings' ) );
	...
	public function set_js_variable() {
		...
		$embeddings_settings = get_option( 'sage_ai_embeddings_settings' );
		...
		$settings_array = array(
			...
			'embeddingSettings'  => $embeddings_settings,
		);
...
<div class="wrap wp-ai-content-gen-settings-wrap">
	...
	public function model_field() {
		...
		<select name="wp_ai_content_gen_settings[model]" >
			<option <?php selected( $value, 'gpt-4-0125-preview', true ); ?> value="gpt-4-0125-preview" >gpt-4 turbo</option>
			<option <?php selected( $value, 'gpt-4', true ); ?> value="gpt-4" > gpt-4</option>
			<option <?php selected( $value, 'gpt-4-32k', true ); ?> value="gpt-4-32k" >gpt-4-32k</option>

			<option <?php selected( $value, 'gpt-3.5-turbo', true ); ?> value="gpt-3.5-turbo" > gpt-3.5-turbo </option>
			<option <?php selected( $value, 'text-davinci-003', true ); ?> value="text-davinci-003" > text-davinci-003 (deprecated)</option>
			<option <?php selected( $value, 'text-davinci-002', true ); ?> value="text-davinci-002" > text-davinci-002 (deprecated)</option>
			<option <?php selected( $value, 'text-curie-001', true ); ?> value="text-curie-001" > text-curie-001 (deprecated)</option>
			<option <?php selected( $value, 'text-babbage-001', true ); ?> value="text-babbage-001" > text-babbage-001 (deprecated)</option>
			<option <?php selected( $value, 'text-ada-001', true ); ?> value="text-ada-001" > text-ada-001 (deprecated)</option>
		</select>
	...
	public function wp_sage_ai_chatbot() {
		?>

			<div class="wrap wp-ai-content-chatbot">
			...
			<!-- Create the form that will be used to render our options -->
			<form method="post" action="options.php">
				<div id="sage-ai-chatbot-backend"></div>
			</form>
	...
	public function wp_sage_ai_images() {
		?>

		<div class="wrap wp-sage-ai-images">

		<!-- Make a call to the WordPress function for rendering errors when settings are saved. -->
				<?php settings_errors(); ?>

		<!-- Create the form that will be used to render our options -->
		<div id="wp-sage-ai-image-generator-backend"></div>
		<!-- <form method="post" action="options.php">
		</form> -->
	...
	public function wp_sage_ai_embeddings() {
		?>

		<div class="wrap wp-sage-ai-embeddings">

		<!-- Make a call to the WordPress function for rendering errors when settings are saved. -->
				<?php settings_errors(); ?>

		<!-- Create the form that will be used to render our options -->
		<div id="wp-sage-ai-embeddings-backend"></div>
		<!-- <form method="post" action="options.php">
		</form> -->
