...
add_action( 'wp_ajax_sage_ai_queue_processor_action', 'sage_ai_queue_processor_action' );
...
/**
 * ajax call to stop the processor.
 *
 * @return void
 */
function sage_ai_queue_processor_action() {
	...
	wp_send_json_success( $bulk_queue_processor );
}
 ...
 * @return void
 */
function sage_ai_generate_post( $content = '', $title = '', $status, $type, $categories = array(), $post_publish_date = '', $taxonomies = array(), $featured_image = array() ) {
	...
	if ( ! empty( $featured_image ) ) {
		$attachment_id = attachment_url_to_postid( $featured_image['text'] );
