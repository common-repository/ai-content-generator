...
add_action( 'wp_ajax_nopriv_sage_ai_call_open_ai', 'sage_ai_call_open_ai' );
...
function sage_ai_call_open_ai( $prompt = '', $args = array() ) {
	...
	$response_type = isset( $_POST['response'] ) ? $_POST['response'] : 'message';
	...
	if ( $model === 'gpt-3.5-turbo' || $model === 'gpt-3.5-turbo-16k' || $model === 'gpt-4' || $model === 'gpt-4-32k' || $model === 'gpt-4-1106-preview' || $model === 'gpt-4-0125-preview' ) {

		// in case of chatbot we send json encoded object.
		$prompt = stripslashes( $prompt );

		if ( json_decode( $prompt ) != null ) {
		...
		$message = sage_ai_call_open_ai_chat_completion( $open_ai_call_args, $open_ai, $is_ajax, $response_type );
		return $message;
...
// Chat Completion
function sage_ai_call_open_ai_chat_completion( $args, $open_ai, $is_ajax, $response_type ) {

	$completion = $open_ai->chat( $args );

	$response = json_decode( $completion );

	if ( $response_type === 'complete' ) { // in case response is complete.
	...
	} elseif ( isset( $response->choices[0]->message->content ) ) { // in case response type is message and api response is success.
	...
	} else {  // in case response type is message and api response is error.
		if ( $is_ajax ) {
